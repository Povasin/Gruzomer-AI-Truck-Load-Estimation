...
def fit_model(args, dataset, normalization, device, validation_dataset=None, epochs=None):
    ...
    for epoch in range(total_epochs):
        ...
        for batch in train_loader:
            ...
            nn.utils.clip_grad_norm_(model.parameters(), 5.0, error_if_nonfinite=False)
...
def train(args):
    ...
    contract = build_contract(
        args.truck_weights,
        args.floor_weights,
        getattr(args, "ceiling_weights", None),
    )
...
def predict(args):
    ...
    ceiling = getattr(args, "ceiling_weights", None) or resolve_saved_path(
        saved_contract["ceiling_weights"]
    )
    contract = build_contract(truck, floor, ceiling)
